...
import { Animated, Dimensions, GestureResponderEvent, View, ScrollView } from 'react-native';
...
export function AppPlants() {
  ...
  function handleResponderRelease(event: GestureResponderEvent) {
    ...
    if (pageY >= medium) {
      Animated
        .spring(pan, { toValue: INITIAL_Y, useNativeDriver: true })
        .start();
    } else {
      Animated
        .spring(pan, { toValue: FINAL_Y, useNativeDriver: true })
        .start();
    }
  }
